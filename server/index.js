...
import { threadToMd, userCommentsToMd } from './md.js';
...
app.get('/api/user-comments', async (req, res) => {
  try {
    const { user, limit = 50, after, include, hide_auto, hide_removed } = req.query;
    ...
    res.json({ markdown, comments, after: nextAfter, count: comments.length });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: error.message });
  }
